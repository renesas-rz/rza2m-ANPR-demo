...
typedef struct {
	...
	int_t SW4_ModeCnt     ; // actual cnt (0,1) by default (controlled by boardKeysInside.h, SW4_ModeCntMax)
	int_t SW4_IsModified  ; // true --> new SW4_ModeCnt available (set to false to allow next key read)
	int_t SW4_ModeCntMax  ; // max count for SW4_ModeCnt

	int_t SW5_ModeCnt     ; // actual cnt (0,1) by default (controlled by boardKeysInside.h, SW5_ModeCntMax)
	int_t SW5_IsModified  ; // true --> new SW5_ModeCnt available (set to false to allow next key read)
	int_t SW5_ModeCntMax  ; // max count for SW4_ModeCnt
