...
void show_welcome_msg (FILE *p_out, bool_t clear_screen)
{
    st_os_abstraction_info_t ver_info;
   	...
   	fprintf(p_out,"\r\n\n################################################################################\n\n");
	fprintf(p_out,"\rRZ/A2M Automatic number-plate recognition (ANPR) for GCC Ver. %u.%u\n", APPLICATION_INFO_VERSION, APPLICATION_INFO_RELEASE);
	fprintf(p_out,"\rCopyright (C) 2021 Renesas Electronics Corporation. All rights reserved.\n");
	fprintf(p_out,"\rBuild Info Date %s at %s \n", __DATE__, __TIME__);

	if(R_OS_GetVersion(&ver_info) == 0)
	{
		fprintf(p_out,"\r%s Version %d.%d\n\n\n", ver_info.p_szdriver_name, ver_info.version.sub.major, ver_info.version.sub.minor);
	}
