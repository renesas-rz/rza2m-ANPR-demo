...
static st_r_drv_gpio_pin_rw_t s_p01_on =
{
    GPIO_PORT_0_PIN_1,
    GPIO_LEVEL_HIGH,
    GPIO_SUCCESS
...
static st_r_drv_gpio_pin_rw_t s_p01_off =
{
    GPIO_PORT_0_PIN_1,
    GPIO_LEVEL_LOW,
    GPIO_SUCCESS
...
static st_r_drv_gpio_pin_rw_t s_p03_on =
{
	GPIO_PORT_0_PIN_3,
    GPIO_LEVEL_HIGH,
    GPIO_SUCCESS
...
static st_r_drv_gpio_pin_rw_t s_p03_off =
{
	GPIO_PORT_0_PIN_3,
    GPIO_LEVEL_LOW,
    GPIO_SUCCESS
...
static st_r_drv_gpio_pin_rw_t s_p05_on =
{
	GPIO_PORT_0_PIN_5,
    GPIO_LEVEL_HIGH,
    GPIO_SUCCESS
...
static st_r_drv_gpio_pin_rw_t s_p05_off =
{
	GPIO_PORT_0_PIN_5,
    GPIO_LEVEL_LOW,
    GPIO_SUCCESS
...
static st_r_drv_gpio_pin_rw_t s_p82_on =
{
	GPIO_PORT_8_PIN_2,
    GPIO_LEVEL_HIGH,
    GPIO_SUCCESS
...
static st_r_drv_gpio_pin_rw_t s_p82_off =
{
	GPIO_PORT_8_PIN_2,
    GPIO_LEVEL_LOW,
    GPIO_SUCCESS
...
void cpuBoardLedGreen ( int_t drive ) {
	if ( drive ) {
		direct_control(s_my_gpio_handle, CTL_GPIO_PIN_WRITE, &s_p01_on);
	} else {
		direct_control(s_my_gpio_handle, CTL_GPIO_PIN_WRITE, &s_p01_off);
	}
}


/**********************************************************************************************************************
 * Function Name: cpuBoardLedYellow
 * Description  : switch the green LED on/off (if the red LED is on the green LED looks like yellow)
 * Arguments    : drive (0-off, 1-on)
 * Return Value : -
 *********************************************************************************************************************/
void cpuBoardLedYellow ( int_t drive ) {
	if ( drive ) {
		direct_control(s_my_gpio_handle, CTL_GPIO_PIN_WRITE, &s_p03_on);
	} else {
		direct_control(s_my_gpio_handle, CTL_GPIO_PIN_WRITE, &s_p03_off);
	}
}

/**********************************************************************************************************************
 * Function Name: cpuBoardLedOrange
 * Description  : switch the green LED on/off (if the red LED is on the green LED looks like yellow)
 * Arguments    : drive (0-off, 1-on)
 * Return Value : -
 *********************************************************************************************************************/
void cpuBoardLedOrange ( int_t drive ) {
	if ( drive ) {
		direct_control(s_my_gpio_handle, CTL_GPIO_PIN_WRITE, &s_p05_on);
	} else {
		direct_control(s_my_gpio_handle, CTL_GPIO_PIN_WRITE, &s_p05_off);
	}
}
...
void cpuBoardLedRed ( int_t drive ) {
	if ( drive ) {
		direct_control(s_my_gpio_handle, CTL_GPIO_PIN_WRITE, &s_p82_on);
	} else {
		direct_control(s_my_gpio_handle, CTL_GPIO_PIN_WRITE, &s_p82_off);
	}
}
