...
void sample_main(void)
{
    ...
    while (1)
    {
        ...
        /* toggle neural network type if SW3 has been pushed */
...
//			    }
		...
		/* check usage of push-button SW2  */
        ...
        if (MAIN_PRV_LED_ON == gs_main_led_flg) {
            switch (prediction_color) {
				case GREEN :
					cpuBoardLedGreen   ( 1 ) ;  // green  led on
					break;
				case YELLOW :
	                cpuBoardLedYellow  ( 1 ) ;  // yellow led on
					break;
				case RED :
	                cpuBoardLedRed     ( 1 ) ;  // red    led on
					break;
            }
        } else {
            cpuBoardLedRed    ( 0 ) ;  // red    led off
