...
#define     LAYER0_DISP_AREA_HS         ((LCD_CH0_DISP_HW - R_BCD_LY2_WIDTH) / 2)
//#define     LAYER0_DISP_AREA_VS         (0u)
#define     LAYER0_DISP_AREA_VS         ((LCD_CH0_DISP_VW - R_BCD_LCD_HEIGHT) / 2)
...
#define     LAYER2_DISP_AREA_HS         ((LCD_CH0_DISP_HW - R_BCD_LY2_WIDTH) / 2)
// #define     LAYER2_DISP_AREA_VS         (0u)
#define     LAYER2_DISP_AREA_VS         ((LCD_CH0_DISP_VW - R_BCD_LCD_HEIGHT) / 2)
#define     LAYER2_DISP_AREA_HW         (R_BCD_LY2_WIDTH)
#define     LAYER2_DISP_AREA_VW         (R_BCD_LY2_HEIGHT)
...
******************************************************************************/
static uint32_t video_buffer[VIDEO_BUFFER_NUM][(VIDEO_BUFFER_STRIDE * VIDEO_BUFFER_HEIGHT)] __attribute__ ((section("Video_RAM")));
...
static uint8_t graphics2_buffer[GRAPHICS2_BUFFER_NUM][(GRAPHICS2_BUFFER_STRIDE * GRAPHICS2_BUFFER_HEIGHT)] __attribute__ ((section("Uncache_IRAM")));
...
/******************************************************************************
* Function Name: R_BCD_LcdGetvisableVramAddress
* Description  : Acquire the address of the video buffer which is currently being displayed
* Arguments    : -
* Return Value : Video buffer 0 address
******************************************************************************/
uint8_t *R_BCD_LcdGetvisableVramAddress(void)
{
    uint8_t *paddr;
...
int32_t R_BCD_LcdInit(void)
{
    ...
    if (error == VDC_OK)
    {
        ...
        gr_disp_cnf.fb_stride = VIDEO_BUFFER_STRIDE*4;        /* Frame buffer stride             */
//        gr_disp_cnf.fb_stride = VIDEO_BUFFER_STRIDE;        /* Frame buffer stride             */
        ...
        gr_disp_cnf.read_format   = VDC_GR_FORMAT_RGB888;           /* Read Format                */
//        gr_disp_cnf.read_format   = VDC_GR_FORMAT_CLUT8;           /* Read Format                */
...

    {
        ...
        /* gra draw init */
        info.p_buff_addr1      = graphics2_buffer[0];
        info.p_buff_addr2      = graphics2_buffer[1];
...
void R_BCD_LcdWriteChar(uint8_t code, uint32_t x, uint32_t y, uint8_t color)
{
    ...
    for (height = 0; height < R_BCD_FONTDATA_HEIGHT; height++)
    {
        check_bit = 7;
        width_offset = 0;
        font = g_font_array[code-0x20][height][width_offset++];
